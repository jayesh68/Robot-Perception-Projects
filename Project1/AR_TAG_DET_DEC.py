...
def TagId(image):
    ...
    if(flag == True):      
        ...
        if (angle_value == 0):
            Id_bin=[tag_matrix[4][3],tag_matrix[4][4],tag_matrix[3][4],tag_matrix[3][3]]
            Id_dec = tag_matrix[3][3]*1 + tag_matrix[3][4]*2 +tag_matrix[4][4]*4 +tag_matrix[4][3]*8  
        elif(angle_value == -90):
            Id_bin=[tag_matrix[3][3],tag_matrix[4][3],tag_matrix[4][4],tag_matrix[3][4]]
            Id_dec = tag_matrix[3][3]*8 + tag_matrix[3][4]*1 + tag_matrix[4][4]*2 +tag_matrix[4][3]*4
        elif(angle_value == 180):
            Id_bin=[tag_matrix[3][4],tag_matrix[3][3],tag_matrix[4][3],tag_matrix[4][4]]
            Id_dec = tag_matrix[3][3]*4 + tag_matrix[4][3]*2 + tag_matrix[4][4] + tag_matrix[3][4]*8
        elif(angle_value == 90):
            Id_bin=[tag_matrix[4][4],tag_matrix[3][4],tag_matrix[3][3],tag_matrix[4][3]]
            Id_dec = tag_matrix[3][3]*2 + tag_matrix[3][4]*4 + tag_matrix[4][4]*8 +tag_matrix[4][3]*1
            
        print('Binary ID',Id_bin)
        return flag, angle_value, Id_dec
...
out = cv2.VideoWriter('multiTagdet.avi',cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
while(cap.isOpened()):# and count!=1
    ...
    for corn in corner_points:
        ...
        for i in range(1,9):
            cv2.line(im_out, (int(rows/8*i),0), (int(rows/8*i), rows*i), (125, 0, 0), 1, 1) 
        for i in range(1,9):
            cv2.line(im_out, (0,int(cols/8*i)), (cols,int(cols/8*i)), (125, 0, 0), 1, 1) 
        
        cv2.imshow('Grid image',im_out)
        #Finding the TAG ID using the new image consisting of the AR tag
        l,m,n = TagId(im_out)
        cv2.putText(frame, str(n),((corn[0][0]-50, corn[0][1]-50)), cv2.FONT_ITALIC, 2, (255,0,255),3)
        cv2.imshow('TagID', frame)   #Displaying the image output in a window named "Display"
